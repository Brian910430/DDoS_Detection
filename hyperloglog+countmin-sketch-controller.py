...
class HLLCMController(object):
    ...
    def __init__(self, sw_name, set_hash):
        ...
        self.topo = Topology(db="/home/p4ora/ddos-detection-sketches-p4/topology.db")
    ...
    def read_hyperloglog_registers(self, get_inactive,num):
        self.hyperloglog_registers = []
        now = 'hyperloglog_sketch' + str(num)
        self.hyperloglog_registers.append(self.controller.register_read(now))
    ...
    def decode_hyperloglog_registers(self, m, get_inactive,num):
        self.read_hyperloglog_registers(get_inactive,num)
...
if __name__ == "__main__":
    ...
    if args.option == "decode_hyperloglog":
        for i in range(3):
	    if controller.controller.register_read('hyper_flag', i) == 1:
		controller.controller.register_write("hyper_flag", i, 0)
		dstIP = controller.controller.register_read('blacklist', i)
		num = controller.decode_hyperloglog_registers(args.hyperloglog_registers, args.get_inactive, i)
		print("{}: {} now has {} diffrent source.\n".format(datetime.now().strftime("%Y-%m-%d %H:%M:%S"), decimal_to_ipv4(dstIP), num))
		with open("logs/forChart.txt", "a") as f:
		     f.write("{} {} {}\n".format(decimal_to_ipv4(dstIP), datetime.now().strftime("%M %S"), num))
