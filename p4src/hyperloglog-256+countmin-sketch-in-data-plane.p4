...
#define HYPERLOGLOG_DDOS_THRESHOLD 1579500000 
// equiv. to 1000, calculated as (1000/(0.7213/(1 + 1.079/256) * 256^2))^(-1) << 25
...
#define HLL_EST_ADD_REGISTER(n) hyperloglog_sketch0.read(hll_value, n); \
 if (hll_value == 0) \
 { \
    hll_sum = hll_sum + (1 << HYPERLOGLOG_MAX_RHO); \
...
control MyEgress(inout headers hdr,
    ...
    apply {
        if (standard_metadata.instance_type == 1) {
            		hdr.ethernet.etherType = 0x1234; // If ingress clone
        }
    }
}
